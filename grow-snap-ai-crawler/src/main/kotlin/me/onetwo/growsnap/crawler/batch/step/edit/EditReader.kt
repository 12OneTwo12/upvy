...
class EditReader(
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(EditReader::class.java)
    }
    ...
    private fun initialize() {
        logger.info("ANALYZED 상태 Job 조회 시작")

        val analyzedJobs = aiContentJobRepository.findByStatus(JobStatus.ANALYZED)
        logger.info("ANALYZED 상태 Job 수: {}", analyzedJobs.size)

        jobs = analyzedJobs.iterator()
    }
}
