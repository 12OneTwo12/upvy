    ...
    private fun buildQuizGenerationPrompt(
        ...
        val goodExamples = when (contentLanguage) {
            ContentLanguage.KO -> """
                |좋은 예시 (명확하고 구체적):
                |영상: "주피터 노트북 사용법"
                |질문: "Python을 웹에서 바로 실행하려면?"
                |보기: "Jupyter", "메모장", "Word", "카톡"
                |→ 맥락 명확! 정답 하나뿐! (Jupyter 외엔 웹에서 실행 안됨)
                |
                |영상: "Git 기초"
                |질문: "코드 저장소로 가장 많이 쓰는 곳은?"
                |보기: "GitHub", "Google Drive", "Dropbox", "USB"
                |→ 맥락 명확! GitHub 외엔 코드 저장소 아님!
                |
                |영상: "웹 개발 입문"
                |질문: "웹사이트 만들 때 반드시 필요한 언어는?"
                |보기: "HTML", "Python", "Java", "C++"
                |→ 명확! HTML만 필수, 나머지는 선택!
                |
                |영상: "VSCode 사용법"
        ...
        return """
            ...
            |
            |2. **명확성 = 맥락 + 구체적 상황**
            |   - 질문만 봐도 무슨 상황인지 정확히 알 수 있어야 함
            |   - 전문 용어는 OK, BUT 반드시 맥락과 함께!
            |   - ❌ "버전 관리 도구는?" → ✅ "코드 변경 이력을 저장하려면?"
            |   - ❌ "커널 재시작은?" → ✅ "Jupyter에서 코드 실행 초기화하려면?"
            |
            |3. **정답이 명백히 하나뿐 + 보기 범위 명시 (필수!)**
            |   - **질문 시작에 "다음 중", "어느 것이" 같은 한정어 반드시 포함!**
            |   - 한정어 예시: "다음 중 ~", "어느 것이 ~", "무엇으로 ~"
            ...
            |7. **정답의 정확성 (절대적!)**
            |   - 정답은 반드시 사실에 근거해야 함
            |   - 질문과 답의 품사/의미가 정확히 일치해야 함
            |   - 틀린 정보를 정답으로 표시하지 말 것
            |   - 확실하지 않은 내용은 질문하지 말 것
            |
            |   **품사/의미 일치 예시:**
            |   - ❌ "컴퓨터와 소통하는 언어는?" → 정답: "프로그래밍" (틀림! 프로그래밍은 행위)
            |   - ✅ "컴퓨터와 소통하는 언어는?" → 정답: "Python" (맞음! 언어 물어봄 → 언어로 답함)
            |   - ✅ "컴퓨터에 명령하는 작업은?" → 정답: "프로그래밍" (맞음! 작업 물어봄 → 작업으로 답함)
            |
            |   **사실 정확성 예시:**
            |   - ❌ "Python 웹 실행?" → 정답: "메모장" (거짓!)
            |   - ✅ "Python 웹 실행?" → 정답: "Jupyter" (사실!)
            |
            |**절대 금지사항:**
            |❌ "이 영상의..." / "영상에서..." 같은 표현
            ...
            |{
            |  "question": "다음 중/어느 것이/무엇으로 + 궁금증 유발 질문 (한정어 필수! 15-30자)",
