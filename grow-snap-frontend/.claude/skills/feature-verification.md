...
# 설치 실패 시 수동으로 APK 설치
adb install -r /Users/jeongjeong-il/Desktop/projects/grow-snap/grow-snap-frontend/android/app/build/outputs/apk/debug/app-debug.apk
```
...
```bash
# GrowSnap 앱이 설치되어 있으므로 에뮬레이터에서 앱 아이콘 클릭하거나
...
for i in {1..10}; do
  PID=$(adb shell ps | grep growsnap | awk '{print $2}')
  if [ -n "$PID" ]; then
    echo "Found PID: $PID"
    break
  fi
...
echo "📱 Step 6: 앱 실행 대기"
echo "   에뮬레이터에서 GrowSnap 앱을 클릭하거나 자동으로 실행될 때까지 대기..."
for i in {1..20}; do
  PID=$(adb shell ps | grep growsnap | awk '{print $2}')
  if [ -n "$PID" ]; then
    echo "✅ 앱 실행됨 (PID: $PID)"
...
adb logcat --pid=$(adb shell ps | grep growsnap | awk '{print $2}') | grep -E "ReactNativeJS|runtime|Error|TypeError"
